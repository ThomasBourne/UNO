...
namespace UNO
{
    ...
    class Run
    {
        static void Main(string[] args)
        {
            ...
            string[] names = PlayerNames(playerCount);
            Console.Clear();
            Deck deck = new Deck();
            ...
            while (winnerPos == null) //where the game loop starts
            {
                for (int i = 0; i < playerCount; i++)
                {
                    if (winnerPos == null)
                    {
                        Console.WriteLine($"Press any key when {names[i]} is ready");
                        Console.ReadKey(); Console.Clear();
                        string[] cardPlayed = { "", "" };
                        ShowDeck(players[i].Deck(), names[i], pile);
                        ...
                        else if (int.TryParse(reqIndex, out intReqIndex) && intReqIndex - 1 >= 0 && intReqIndex - 1 < players[i].Deck().Count)
                            cardPlayed = players[i].Deck()[intReqIndex - 1];
                        ...
                        if (intReqIndex > 0)
                            players[i].Deck().RemoveAt(intReqIndex - 1); //BOTH REVERSE AND MISS A GO ARE SEVERY BUGGED
                        else
                            players[i].Deck().Add(deck.GenerateCard());
                        Console.Clear();
                        winnerPos = IsWinner(players);
                    }
                }
            }
        ...
        private static string[] PlayerNames(int playerCount)
        {
            string[] names = new string[playerCount];
            for (int i = 0; i < playerCount; i++)
            {
                Console.Write($"Enter the name for player {i + 1}:{Environment.NewLine}>>> ");
